#include <SPI.h>
#include <LoRa.h>
#include <Wire.h>
#include <Adafruit_BME680.h>
#include <WiFi.h> // Для вимкнення Wi-Fi
#include "esp_sleep.h" 
#include "lorawan_config.h"

// --- Об'єкти ---
Adafruit_BME680 bme;

struct __attribute__((packed)) SensorData {
    int16_t temperature; 
    uint16_t humidity;   
    uint16_t pressure;   
    uint16_t voc;        
};
SensorData dataToSend;

// Функція переходу в сон (з кнопкою)
void enterDeepSleep(int seconds) {
  Serial.printf("Йдемо спати на %d секунд...\n", seconds);
  // ДОДАНО: Пробудження від кнопки
  esp_sleep_enable_ext0_wakeup(GPIO_NUM_4, 0); 
  esp_sleep_enable_timer_wakeup(seconds * 1000000ULL);
  delay(200); 
  esp_deep_sleep_start(); 
}

bool readSensorData() {
  delay(1000); // Час на прогрів BME
  if (bme.performReading()) {
      dataToSend.temperature = (int16_t)(bme.temperature * 100.0);
      dataToSend.humidity = (uint16_t)(bme.humidity * 100.0);
      dataToSend.pressure = (uint16_t)(bme.pressure / 100.0);
      dataToSend.voc = (uint16_t)(bme.gas_resistance / 1000.0); 
      return true;
  } else {
      Serial.println("Помилка зчитування BME!");
      return false;
  }
}

void transmitData() {
  if (!readSensorData()) return;

  Serial.printf("Зчитано дані: T=%.2f C, H=%.2f %%, P=%.0f hPa, VOC=%.0f kOhm\n", 
                (float)dataToSend.temperature / 100.0, 
                (float)dataToSend.humidity / 100.0, 
                (float)dataToSend.pressure, 
                (float)dataToSend.voc); 

  Serial.print("Відправка LoRa пакета... ");
  LoRa.beginPacket();
  LoRa.write((uint8_t*)&dataToSend, sizeof(dataToSend));
  LoRa.endPacket();
  Serial.println("Пакет відправлено.");
}

void setup() {
    Serial.begin(115200);
    while (!Serial);
    delay(1000);
    
    WiFi.mode(WIFI_OFF);
    btStop();
    
    Serial.println("\n--- LoRa v2 ---");
    
    pinMode(BUTTON_PIN, INPUT_PULLUP);

    if (!bme.begin()) {
        Serial.println(F("Помилка ініціалізації BME680! Йдемо у сон..."));
        enterDeepSleep(MAIN_INTERVAL / 1000);
    }
    bme.setTemperatureOversampling(BME680_OS_8X);
    bme.setHumidityOversampling(BME680_OS_2X);
    bme.setPressureOversampling(BME680_OS_4X);
    bme.setIIRFilterSize(BME680_FILTER_SIZE_3); 
    bme.setGasHeater(320, 150); 
    Serial.println(F("BME680 ініціалізовано."));

    LoRa.setPins(LORA_CS_PIN, LORA_RST_PIN, LORA_DIO0_PIN);
    if (!LoRa.begin(LORA_FREQUENCY)) { 
        Serial.println("Помилка запуску LoRa! Йдемо у сон...");
        enterDeepSleep(MAIN_INTERVAL / 1000);
    }
    Serial.println("LoRa ініціалізовано.");

    transmitData();
    
    enterDeepSleep(MAIN_INTERVAL / 1000);
}

void loop() {
    // Порожній
}